...
def insert_rows(cur, conn, schema, row):
    try:
        ...
        conn.commit()
        logger.info(f"Inserted row with Video_ID: {row.get(video_id, 'MISSING_VIDEO_ID')}")

    except Exception as e:
        logger.error(f"Error inserting row with Video_ID: {row.get(video_id, 'MISSING_VIDEO_ID')} - {e}")
        raise e
...
def update_rows(cur, conn, schema, row):
    ...
    try:
        ...
        logger.info(f"Updated row with Video_ID: {row.get(video_id, 'MISSING_VIDEO_ID')}")

    except Exception as e:
        logger.error(f"Error updating row with Video_ID: {row.get(video_id, 'MISSING_VIDEO_ID')} - {e}")
        raise e
